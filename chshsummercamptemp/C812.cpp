...
vpii graph[5005];
int n, k, q, a, b, c, vis[5005], minimum = inf;

int DFS(int start, int limit, int mini){
    vis[start] = 1;
    int ans = 0;
    FNAT(i, graph[start]){
        if(!vis[i.ff]){
            vis[i.ff] = 1;
            ans += DFS(i.ff, limit, mi(mini, i.ss));
        }
    }
    if (mini>=limit && mini!=inf)ans+=1;
    return ans;
}
